...
API предоставляет RESTful интерфейс для управления графиком проведения технического воздушных судов (далее ВС) и работами на них по заявкам заказчиков. Для аутентификации используется JWT (JSON Web Token), который необходимо передавать в заголовке `Authorization` в формате `Bearer <token>`.
...
*   **Авторизация:** Требуется Bearer токен.
*   **Параметры:**
    *   `aircraftId` (integer, required): Идентификатор ВС, к которой нужно добавить работы.
*   **Тело запроса:**
...
*   **Возвращаемый ответ (201):**
    ...
    ```
    *   `message` (string):  Сообщение об успешном создании работ.
*   **Ошибка (400):**
