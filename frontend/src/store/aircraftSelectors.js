...
export const selectAircraftsWithTotalManpower = createSelector(
    ...
    (aircrafts, tasks) => {
        ...
        return aircrafts.map(aircraft => {
             const totalManpower = aircraft.tasks.reduce((acc, task) => acc + (task.manpower || 0), 0);
...
export const selectAircraftsForGantt = createSelector([selectAircrafts], (aircrafts) => {
    return aircrafts.map((aircraft) => {
        const startDate = aircraft.start_date ? new Date(aircraft.start_date) : null;
        const endDate = aircraft.finish_date ? new Date(aircraft.finish_date) : null;
